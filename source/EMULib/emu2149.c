...
#ifdef ARM7
EMU2149_API PSG *
PSG_new (PSG * psg, e_uint32 c, e_uint32 r)
{
...
EMU2149_API int 
PSG_serialize(PSG * p, char * buf )
{
	if( buf != NULL ) memcpy( buf , p, sizeof(PSG) );
	return sizeof(PSG);
}

EMU2149_API void
PSG_deserialize(PSG * p, char * buf )
{
	if( buf != NULL ){
		memcpy( p, buf, sizeof(PSG) );
		p->voltbl = voltbl[EMU2149_VOL_DEFAULT];	// Žè”²‚«
	}
}
