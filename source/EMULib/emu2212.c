...
#ifdef ARM7
EMU2212_API SCC *
SCC_new (SCC * scc, e_uint32 c, e_uint32 r)
{
...
EMU2212_API int 
SCC_serialize(SCC * s, char * buf )
{
	if( buf != NULL ) memcpy( buf , s, sizeof(SCC) );
	return sizeof(SCC);
}

EMU2212_API void
SCC_deserialize(SCC * s, char * buf )
{
	if( buf != NULL ){
		memcpy( s, buf, sizeof(SCC) );
	}
}
