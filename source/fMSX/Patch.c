...
byte TapeHeader[8] = { 0x1F,0xA6,0xDE,0xBA,0xCC,0x13,0x7D,0x74 };
...
****************************************************************/
{
  ...
  if(CasName)
  {
	if( CasPtr > 0 ){ ret = LLIST_next( CasData ); CasPtr = 0; }
	if( ret != LLIST_OK ) return;
	R->AF.B.l&=~C_FLAG;
  }
  return;
}
...
//  LOG( "TAPIN" );
  if(CasName)
  {
    TapeData * td = (TapeData*)LLIST_get( CasData );
	if( td ){
//		LOG( "TAPIN ptr[%d]=%X",CasPtr, td->buf[CasPtr] );
		J=td->buf[CasPtr++];
    	R->AF.B.h=J; R->AF.B.l&=~C_FLAG;
    }
  }
  return;
}
...
****************************************************************/
	...
	if(CasName)
	{
		TapeData * otd = (TapeData*)LLIST_get( CasData );
		TapeData * td = NULL;
		if( otd == NULL ) return;
		if( CasPtr >= otd->siz ){
			TapeData * td = (TapeData*)calloc(1, sizeof(TapeData) + otd->siz + 8 );
			memcpy( td->buf, otd->buf, otd->siz );
			td->siz = otd->siz + 8;
			LLIST_set( CasData, td );
		}else{
			td = otd;
		}
		td->buf[CasPtr++] = R->AF.B.h;
		R->AF.B.l&=~C_FLAG;
	}
