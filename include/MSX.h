...
extern int   UseSpriteOver; /* Sprite Over Check   */      
...
#define JOY_MOUSE    2      /* Mouse                         */ 
#define JOY_MOUSTICK 3      /* Mouse acting as joystick      */ 
...
#define MAP_SCC      8      /* SCC Sound cartridge           */
...
#define MAXBIOS     16      /* Max number of bios roms       */
...
extern /*volatile*/ byte MSXKeyMap[16];

//#define KBD_SET(K)   MSXKeyMap[Keys[K][0]]&=~Keys[K][1]
//#define KBD_RES(K)   MSXKeyMap[Keys[K][0]]|=Keys[K][1]
...
/** BIOS settings ***************************************************/
typedef struct {
	char * name;                          /* BIOS filename          */
	byte PS;                              /* Primary slot number    */
	byte SS;                              /* Secondry slot number   */
	byte PG;                              /* Puting Page top        */
	int siz;                              /* BIOS data size         */
} BIOS;
...
extern int  VPeriod;                      /* CPU cycles per VBlank  */
extern int  HPeriod;                      /* CPU cycles per HBlank  */
extern int  RAMPages;                     /* Number of RAM pages    */
extern int  VRAMPages;                    /* Number of VRAM pages   */
extern byte Power;                        /* Power status           */

/** Main hardware: CPU, RAM, VRAM, mappers **************************/
extern Z80  CPU;                          /* CPU state/registers    */

extern byte *VRAM,*VPAGE;                 /* Video RAM              */

extern byte *RAM[8];                      /* Main RAM (8x8kB pages) */
extern byte *EmptyRAM;                    /* Empty RAM page (8kB)   */
extern byte SaveCMOS;                     /* Save CMOS.ROM on exit  */
extern byte *MemMap[4][4][8];   /* Memory maps [PPage][SPage][Addr] */

extern byte *RAMData;                     /* RAM Mapper contents    */
extern byte RAMMapper[4];                 /* RAM Mapper state       */
extern byte RAMMask;                      /* RAM Mapper mask        */

extern byte *ROMData[MAXSLOTS];           /* ROM Mapper contents    */
extern byte ROMMapper[MAXSLOTS][4];       /* ROM Mappers state      */
extern byte ROMMask[MAXSLOTS];            /* ROM Mapper masks       */
extern byte ROMType[MAXSLOTS];            /* ROM Mapper types       */

extern byte EnWrite[4];                   /* 1 if write enabled     */
extern byte PSL[4],SSL[4];                /* Lists of current slots */
extern byte PSLReg,SSLReg[4];   /* Storage for A8h port and (FFFFh) */

extern BIOS bios[MAXBIOS];                /* BIOS settings          */

extern byte ChrGenType;                  /* BIOS CharcterGen. types */
extern byte DateFormatType;              /* BIOS Date format types  */
extern byte KeyboardType;                /* BIOS Keyboard types     */
...
extern char *ROMName[MAXSLOTS];           /* Cartridge file name    */

/** On-cartridge SRAM data ******************************************/
extern char *SRAMName[MAXSLOTS];          /* Filenames (gen-d)      */
extern byte  SaveSRAM[MAXSLOTS];          /* Save SRAM on exit      */
extern byte *SRAMData[MAXSLOTS];          /* SRAM (battery backed)  */
...
extern LLIST * CasData ;
extern int CasPtr   ;
//extern FILE* CasStream;                   /* Cassette I/O stream    */

/** Serial port *****************************************************/
extern char *ComName;                     /* Serial redir. file     */
//extern  FILE *ComIStream;
//extern  FILE *ComOStream;

/** Kanji font ROM **************************************************/
extern char *KanjiName;                   /* Kanji.ROM file         */
extern byte *Kanji;                       /* Kanji ROM 4096x32      */
extern int  KanLetter;                    /* Current letter index   */
extern byte KanCount;                     /* Byte count 0..31       */

/** CMOS data  ******************************************************/
extern char *CMOSName;                    /* CMOS data name         */

/** Keyboard, joystick, and mouse ***********************************/
extern /*volatile*/ byte MSXKeyMap[16];   /* Keyboard map           */
extern word JoyState;                     /* Joystick states        */
extern int  MouState[2];                  /* Mouse states           */
extern byte MouseDX[2],MouseDY[2];        /* Mouse offsets          */
extern byte OldMouseX[2],OldMouseY[2];    /* Old mouse coordinates  */
extern byte MCount[2];                    /* Mouse nibble counter   */

/** General I/O registers: i8255 ************************************/
extern I8255 PPI;                         /* i8255 PPI at A8h-ABh   */
extern byte IOReg;                        /* Storage for AAh port   */

/** Disk controller: WD1793 *****************************************/
extern WD1793 FDC;                        /* WD1793 at 7FF8h-7FFFh  */
extern FDIDisk FDD[4];                    /* Floppy disk images     */

/** Sound hardware: PSG, SCC, OPLL **********************************/
//extern PSG * psg;
extern byte PSGLatch;

//extern SCC * scc;
extern byte SCCOn[2];                     /* 1 = SCC page active    */
extern int  SCCPSlot;                     /* SCC+ cart slot No.     */
extern byte *SCCRAMData;                  /* SCC Mapper RAM data    */
extern byte SCC_RAM_MODE[4];              /* SCC RAM Mode           */

//extern OPLL* opll;
extern byte OPLLLatch;
extern word FMPACKey;                     /* MAGIC = SRAM active    */

extern int  SampleRate;                   /* wave form sample rate  */
...
/** Real-time clock *************************************************/
extern byte RTCReg,RTCMode;               /* RTC register numbers   */
...
extern word VAddr;                        /* VRAM address in VDP    */
extern byte VKey,PKey,WKey;               /* Status keys for VDP    */
...
extern byte IRQPending;                   /* Pending interrupts     */
extern int  ScanLine;                     /* Current scanline       */
extern byte VDPData;                      /* VDP data buffer        */
extern byte PLatch;                       /* Palette buffer         */
extern byte ALatch;                       /* Address buffer         */
extern int  Palette[16];                  /* Current palette        */

/** Places in DiskROM Slot 0:PS 1:SS 2:PAGE ******************/
extern int  PatchSlot[3];

/********************************************************************/
int InitMSX(void);
int bootMSX(void);
int initPallette(void);
int loadMSXDOS2(int slot, char * fn );
int loadSCCSound(int slot);
int loadFMPAC(int slot, char * fn );
int loadCMOS(char * fn );
int patchBDOS(int a, int b, int c);
int patchBIOS(void);
int loadKanjiRom( char * fn, int siz );
int setBios(int a, int b, int c, char * fn, int siz );
...
/************************************ TO BE WRITTEN BY USER **/
extern void RefreshLineTx80(byte Y);
extern void RefreshLine0(byte Y);
extern void RefreshLine1(byte Y);
extern void RefreshLine2(byte Y);
extern void RefreshLine3(byte Y);
extern void RefreshLine4(byte Y);
extern void RefreshLine5(byte Y);
extern void RefreshLine6(byte Y);
extern void RefreshLine7(byte Y);
extern void RefreshLine8(byte Y);
extern void RefreshLine10(byte Y);
extern void RefreshLine12(byte Y);

extern void RefreshLine6W(byte Y);
extern void RefreshLine7W(byte Y);
extern void RefreshLineTx80W(byte Y);

void RefreshLine0_x2(byte Y);
void RefreshLine1_x2(byte Y);
void RefreshLine2_x2(byte Y);
void RefreshLine3_x2(byte Y);
void RefreshLine4_x2(byte Y);
void RefreshLine5_x2(byte Y);
void RefreshLine6_x2(byte Y);
void RefreshLine7_x2(byte Y);
void RefreshLine8_x2(byte Y);
void RefreshLine10_x2(byte Y);
void RefreshLine12_x2(byte Y);
void RefreshLineTx80_x2(byte Y);
